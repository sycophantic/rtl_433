...
static int ambient_weather_tx3102_decode(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    int channel;
    float temperature;
    int moisture_array[16] = { 0, 7, 13, 20, 27, 33, 40, 47, 53, 60, 67, 73, 80, 87, 93, 99 }; //device has 16 values mapped from 0% to 99%
    int moisture;
    int battery_ok;
    int sign;
    uint16_t crc;
    ...
    crc = crc16(dataforcrc, 15, 0x1021, 0x0000);
    ...
    if (channel > 7) { // channel can only be 1 to 7
